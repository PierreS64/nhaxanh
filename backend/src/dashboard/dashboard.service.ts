...
export class DashboardService {
  ...
  async getLandlordMetrics(userId: string) {
    ...
    const startOfMonthVn = new Date(
      Date.UTC(nowVn.getUTCFullYear(), nowVn.getUTCMonth(), 1, 0, 0, 0),
    );
    const startOfNextMonthVn = new Date(
      Date.UTC(nowVn.getUTCFullYear(), nowVn.getUTCMonth() + 1, 1, 0, 0, 0),
    );
    ...
    const [revenueResult, unpaidInvoicesCount, activeContractsCount] =
      await Promise.all([
