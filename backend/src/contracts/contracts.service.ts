...
export class ContractsService {
  ...
  async findAll(userId: string, role: string) {
    if (role === Role.TENANT) {
      ...
      return this.prisma.contract.findMany({
        include: {
          Property: true,
          User_Contract_tenantIdToUser: true,
          User_Contract_landlordIdToUser: true,
        },
      });
    }
    return [];
  ...
  async findOne(id: string, userId: string, role: string) {
    ...
    if (role === Role.TENANT && contract.tenantId !== userId)
      throw new ForbiddenException('Access denied');
    if (role === Role.LANDLORD && contract.landlordId !== userId)
      throw new ForbiddenException('Access denied');
